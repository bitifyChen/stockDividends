...
const filteredDividends = computed(() => dividendList.value.filter((item) => item.year === selectedYear.value))
const yearTotal = computed(() => filteredDividends.value.reduce((total, item) => add(total, multiply(item.earn, item.stockNum)), 0))
const allTimeTotal = computed(() => dividendList.value.reduce((total, item) => add(total, multiply(item.earn, item.stockNum)), 0))
  ...
  <div class="terminal-page">
    ...
    <section class="terminal-panel" v-loading="loading">
      ...
      <el-table :data="filteredDividends" style="width: 100%" class="terminal-el-table">
        ...
        <el-table-column :label="shareColumnLabel(dashboardSettingStore.shareUnit, '持有')" align="right" width="120">
          <template #default="{ row }">{{ formatShare(row.stockNum, dashboardSettingStore.shareUnit) }}</template>
        </el-table-column>
...
.terminal-titlebar, .terminal-panel, .ticker-card { border: 1px solid #2f3339; border-radius: 6px; background: #1b1d21; }
...
.terminal-select { display: grid; gap: 6px; color: #94a3b8; font-size: 12px; font-weight: 900; }
.terminal-select select { height: 38px; min-width: 140px; border: 1px solid #30343a; border-radius: 6px; background: #111317; color: #d4d8dd; padding: 0 10px; }
.market-strip { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }
.ticker-card { padding: 14px; }
.ticker-head { display: flex; align-items: center; justify-content: space-between; color: #94a3b8; font-size: 12px; font-weight: 900; }
.ticker-card strong { display: block; margin-top: 10px; color: #f7fafc; font-size: 22px; font-weight: 900; }
...
.panel-head span, .date-text { color: #94a3b8; font-size: 13px; font-weight: 800; }
.stock-cell { display: grid; gap: 3px; }
...
@media (max-width: 900px) {
  ...
  .terminal-titlebar { align-items: stretch; flex-direction: column; }
}
</style>
